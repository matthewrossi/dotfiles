function strip-metadata --description 'Convert text pdfs into images pdfs and strip metadata' --argument filename
	if test -z $filename; or not test -e $filename
		echo "Usage: `strip-metadata filename.pdf` or `strip-metadata dirname`"
		return 1
	end
